...
int main(int argc, char ** argv){
    ...
    int r1 = atoi(argv[3]); //垂直方向，慢的变化的
    int r2 = atoi(argv[4]); //水平方向，快的变化的
    int time_dim = atoi(argv[5]);
    double max_eb = atof(argv[6]);
    // int option = 0;
    // int retry = 3;
    // if(argc > 7) option = atoi(argv[7]);
    // if(argc > 8) retry = atoi(argv[8]);
    ...
    // result = fff_compress_2d_fp(U, V, r1, r2, time_dim, result_size, false, max_eb);
    // exit(0);
    // result = sz_compress_cp_preserve_sos_2d_time_online_fp(U, V, r1, r2,time_dim, result_size, false, max_eb);
