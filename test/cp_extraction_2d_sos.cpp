...
int main(int argc, char **argv){
  ...
  uint64_t vector_field_scaling_factor = static_cast<uint64_t>(std::stoull(argv[3]));
  int DW = atoi(argv[4]); //faster dim 640
  int DH = atoi(argv[5]); //slowest dim 80
  ...
  free(u);
  free(v);
  //print out critical points locations
  for (const auto& p:critical_points_0){
    auto cp = p.second;
    std::cout << "Simplex " << cp.simplex_id << " Type " << get_critical_point_type_string(cp.type) 
      << " Position (" << cp.x[0] << ", " << cp.x[1] << ")" << std::endl;
  }
